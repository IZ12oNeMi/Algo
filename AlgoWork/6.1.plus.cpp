...
int path[cnum][cnum+1] = {0}; // 标记路径
int length[cnum] = {0}; // 路径总长
int N = 1;  //记录走过的城市 
...
void greedy_DFS(int start, int i) //贪心遍历
{
    ...
    {
        path[start][cnum] = start;
        length[start] += dis[i][start];
    }
}
...
void proceed()
{
    ...
    cout<<path[index][cnum]<<endl;
    cout<<"The minimum of length is :"<<length[index]<<endl;
}
